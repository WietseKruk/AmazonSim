...
public class Vrachtwagen implements Object3D, Updatable {
    ...
    public boolean update() {
        ...
        // als de vrachtwagen spullen aan het afhalen is totdat deze vol is. wordt
        // meerdere keren uitgevoerd
        if (productCounter < 10 && x == laadStationX && z == laadStationZ && !isDelivering()
                && world.isRobotAvailable()) {

            // extra check om timingsproblemen te voorkomen
            if (stellageTarget < 10) {
                world.commandRobot(getPossibleDestinations().get(stellageTarget), isDelivering); // isRobotDelivering
                                                                                                 // moet in world
                                                                                                 // meegegeven worden
                                                                                                 // aan de robot zodat
                                                                                                 // deze daar gebruikt
        ...
        // als de vrachtwagen aan het afhalen is en deze is nu vol. wordt maar 1 keer
        // uitgevoerd
        if (productCounter == 10 && /* x == laadStationX && z == laadStationZ && */!isDelivering()) {
        ...
        // als de vrachtwagen aan het bezorgen is en deze heeft nog producten, wordt
        // meerdere keren uitgevoerd
        if (productCounter > 0 && x == laadStationX && z == laadStationZ && isDelivering()
                && world.isRobotAvailable()) {
            world.commandRobot(getPossibleDestinations().get(stellageTarget), isDelivering); // in commandRobot() code
                                                                                             // aanpassen gebaseerd op
                                                                                             // isRobotDelivering;
            stellageTarget++;
        ...
        // als de vrachtwagen spullen aan het afleveren is maar deze is nu leeg, wordt
        // maar 1 keer uitgevoerd
        if (productCounter == 0 && x == laadStationX && z == laadStationZ && isDelivering()) {
            isDelivering = false;
            stellageTarget = 0;
            shuffleStellages();
